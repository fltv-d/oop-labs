...
void Base::Input()
{
    switch (type)
    {
        ...
        case 4:
            ...
            break;
        defaul:
            cout << "ERROR: object type unknown!\n";
    }	
}
...
void SubjList::SortingItems()
{
	Base *a;
	char **b;
	int n = 0, i = 0, k = 0, counter = 0, help = 0, m = 0;
	while(n < Count())
	{
		a = ((Base*)GetItem(n));
		if(a->type == 1)
    ...
    if(i > 0)
    {
	    ...
	    while(n < i)
	    {
	    	while(counter < Count())
	    	{
    	    	if(((Base*)GetItem(counter))->type == 1 && strcmp(b[n], ((FIO*)GetItem(counter))->secondName()) == 0)
	        	{
		        	a = ((Base*)GetItem(counter));
		        	if(n == 0 && GetIndex((Item*)a) != 0)
                    {
                    	a = ((Base*)Remove(counter));
                        Insert((Item*)a, 0);
                    }
                    else
                    {
                    ...
                    help = GetIndex((Item*)a) + 1;
                    k = help;
                    while(k < Count())
                    {
                    	if(((Base*)GetItem(m))->personnelNumber == ((Base*)GetItem(k))->personnelNumber && k != n)
                    	{
                    		a = ((Base*)Remove(k));
            	        	Insert((Item*)a, help);
            	        	help = GetIndex((Item*)a) + 1;
